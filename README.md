...
콘텐츠 맵에서는 조회와 `shared_ptr` 복사에 필요한 구간에서만 Shared Lock을 점유합니다. 이후 Map Lock을 해제하고 콘텐츠별 Lock을 획득하여 로직을 실행합니다. 이를 통해 Control Thread의 Exclusive Lock 대기 시간을 줄이면서 실행 중인 콘텐츠의 수명을 안전하게 유지합니다.
